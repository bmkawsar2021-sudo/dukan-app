...
const StockRegister = () => {
  ...
  useEffect(() => {
    ...
    const unsubEntries = subscribeStockEntries(setStockEntries);
    return () => { unsubProducts(); unsubEntries(); };
  }, []);

  const handleAddProduct = async () => {
    if (!newProduct.name.trim()) return;
    await addProduct({ name: newProduct.name, unit: newProduct.unit, openingStock: Number(newProduct.openingStock) });
  ...
  const handleAddEntry = async () => {
    if (!newEntry.productId || Number(newEntry.quantity) <= 0) return;
    await addStockEntry({ ...newEntry, quantity: Number(newEntry.quantity) });
  ...
  return (
    <div>
      ...
      {showAddProduct && (
        <div style={{ ...st.card, marginBottom: 24 }}>
          <div style={st.cardBody}>
            <h3 style={{ fontSize: 17, fontWeight: 700, color: '#f1f5f9', marginBottom: 20 }}>Add New Product</h3>
      ...
      {showStockEntry && (
        <div style={{ ...st.card, marginBottom: 24 }}>
          <div style={st.cardBody}>
            <h3 style={{ fontSize: 17, fontWeight: 700, color: '#f1f5f9', marginBottom: 20 }}>Stock IN / OUT Entry</h3>
      ...
      <div style={{ ...st.card, marginBottom: 24 }}>
        <div style={st.headerBar}>
          <h3 style={{ fontSize: 17, fontWeight: 700, color: '#f1f5f9' }}>Product Stock Overview</h3>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {products.length === 0 ? (
                <tr><td colSpan="7" style={{ ...st.td, textAlign: 'center', padding: 32, color: '#64748b' }}>No products yet. Add your first product!</td></tr>
              ) : (
                products.map((product, idx) => {
                  ...
                  return (
                    <tr key={product.id} style={{ background: idx % 2 === 0 ? '#1e293b' : '#172033' }}
                    ...
                    >
                      ...
                      <td style={{ ...st.tdCenter, color: '#94a3b8' }}>{product.unit}</td>
                      <td style={st.tdRight}>{product.openingStock || 0}</td>
                      <td style={{ ...st.tdRight, color: '#818cf8' }}>+{stockIn}</td>
                      <td style={{ ...st.tdRight, color: '#f87171' }}>-{stockOut}</td>
                      <td style={{ ...st.tdRight, fontWeight: 700, color: isLow ? '#f87171' : '#f1f5f9' }}>
                        {balance}
                        {isLow && <span style={{ marginLeft: 6, fontSize: 10, background: '#7f1d1d', color: '#fca5a5', padding: '2px 6px', borderRadius: 6, fontWeight: 700 }}>LOW</span>}
                      </td>
                      <td style={st.td}>
      ...
      <div style={st.card}>
        <div style={st.headerBar}>
          <h3 style={{ fontSize: 17, fontWeight: 700, color: '#f1f5f9' }}>Recent Stock Entries</h3>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
            ...
            <tbody>
              {stockEntries.length === 0 ? (
                <tr><td colSpan="5" style={{ ...st.td, textAlign: 'center', padding: 32, color: '#64748b' }}>No stock entries yet.</td></tr>
              ) : (
                [...stockEntries].reverse().slice(0, 20).map((entry, idx) => {
                  const product = products.find((p) => String(p.id) === String(entry.productId));
                  return (
                    <tr key={entry.id} style={{ background: idx % 2 === 0 ? '#1e293b' : '#172033' }}
                      onMouseEnter={e => e.currentTarget.style.background = '#0f172a'}
                      onMouseLeave={e => e.currentTarget.style.background = idx % 2 === 0 ? '#1e293b' : '#172033'}
                    >
                      ...
                      <td style={st.tdCenter}>
                        <span style={{
                          ...
                          background: entry.type === 'in' ? '#312e81' : '#7f1d1d',
                          color: entry.type === 'in' ? '#a5b4fc' : '#fca5a5',
                        }}>
